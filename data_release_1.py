...
class virus_count : #Create a class called virus_count
    ...
    def get_date(self): # creating a getter method to get the date of a virus_count object
        return self.date
    ...
    def instantiate_from_csv(cls, filename: str):
        ...
        for row in rows_of_patients:
            #creating a try except block to fix any value errors that may occur when trying to create patient objects from the csv file, if there is a value error it will just skip that row and move on to the next one
            try:    
                virus_count(
                day = int(row['Day']),
                date = row['Date'],
    ...
    def get_virus_count(cls, active_reported_daily_cases):
        for virus_count in virus_count.all_virus_count:
